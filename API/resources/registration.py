...
class Vech_Reg(Resource):
    ...
    def post(self):
        ...
        data=parser.parse_args()
        try:
            test=query(f"""SELECT * FROM CBIT_PARKING.REGISTRATION WHERE vehicle_no='{data['vehicle_no']}' AND reg_status='deleted'""",return_json=False)
            if len(test)>0 :
                return {"message":"This vehicle has been deleted previously. Pay fine to register it again."},400
        except :return {"message":"There was an error inserting into REGISTRATION table."},500
...
class DismissVech(Resource):
    ...
    def put(self):
        ...
        data=parser.parse_args()
        try:
            query(f"""UPDATE CBIT_PARKING.REGISTRATION SET reg_status='dismissed',updated_date='{datetime.date.today(),}' WHERE reg_id={data['reg_id']}""")
    ...
    def get(self):
        try:
            return query(f"""SELECT * FROM CBIT_PARKING.REGISTRATION WHERE reg_status='dismissed'""")
        except:
            return {"message":"There was an error displaying the data"},500 

class DeleteVech(Resource):
    @jwt_required
    def put(self):
        parser=reqparse.RequestParser()
        parser.add_argument('reg_id',type=int,required=True,help="please give reg_id")
        data=parser.parse_args()
        try:
            query(f"""UPDATE CBIT_PARKING.REGISTRATION SET reg_status='deleted',updated_date='{datetime.date.today()}' WHERE reg_id={data['reg_id']}""")
        except:
            return {"message":"there was an error updating the reg_status"},500
        return {"message":"deleted vehicle successfully"},201

class Stats(Resource):
    @jwt_required
    def get(self):
        parser=reqparse.RequestParser()
        parser.add_argument('updated_date',type=str,required=True,help="please give the date")
        data=parser.parse_args()
        try:
            return query(f"""SELECT * FROM CBIT_PARKING.REGISTRATION WHERE updated_date='{data['updated_date']}'""")
        except:
            return {"message":"There was an error displaying the data"},500 

class UserRegs(Resource):
    @jwt_required
    def get(self):
        parser=reqparse.RequestParser()
        parser.add_argument('user_id',type=str,required=True,help="please give the user_id")
        data=parser.parse_args()
        try:
            return query(f"""SELECT * FROM CBIT_PARKING.REGISTRATION WHERE user_id='{data['user_id']}'""")
        except:
            return {"message":"There was an error displaying the data"},500 
